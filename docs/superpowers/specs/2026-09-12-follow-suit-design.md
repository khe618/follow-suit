...
- **Start drops absent seats.** `start-game` first removes any human seat that is disconnected at that moment (its resume token stops working), then requires at least two seats remain. Nobody is dealt into a game they had already left.
...
- **Room deletion.** A room is deleted, with its bots and every timer cleared, when it has had no connected human for `RESUME_TTL_MS`, in any phase. This is the one way a game in progress ends early. A room that has never had a seat (only visitors on the name screen) is deleted as soon as its last visitor disconnects.
