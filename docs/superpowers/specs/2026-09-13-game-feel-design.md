...
`tutorial.js` renders a `<dialog>` with six slides, a dot indicator, Back and Next, keyboard arrows, and swipe on touch. Opened from the **?** button on the landing, the join screen, the lobby, and the table, and by visiting `/how-to-play`. Each slide has a mini table (same seat, card, and chip elements at reduced scale, three seats) and two or three short sentences of caption; no fact pills. Slide animations replay each time the slide is shown.

1. **Objective.** A spade sits on the slot; the top card turns over beside it, also a spade, the rail flashes green, and chips fly from both other seats to you. Caption: "Each round you bet on whether the next card matches the suit on top. Win the bet and every other player pays you 100. Most chips when the deck runs out wins."
2. **Deal.** Cards deal to three seats from a deck counting down from 40; yours flip up. Caption: "Everyone is dealt a hand from a 40-card deck, 10 of each suit. You see only your own."
3. **Shuffle back.** The undealt cards slide off the table, the hands fly back to rebuild the deck, it riffles, the top card flips up. Caption: "The hands are shuffled together into a new deck. The cards nobody was dealt are set aside."
4. **Bid.** Three bid tags rise to 80, 50, 20; the 80 glows gold. Caption: "Everyone bids 0 to 100 in secret. Highest bid buys the bet from everyone else at that price."
5. **Pay.** The same two-step as live play: chips fly 80 from the buyer to each of the two others; the card turns over beside the reference for the comparison beat, then takes the slot; a Match/Miss toggle replays the payout: on a match 100 comes back from each; on a miss nothing does. The slide ends on the net delta badges (`+40 / −20 / −20` or `−160 / +80 / +80`). Caption: "Match: each other player pays the buyer 100. Miss: the buyer keeps nothing."
6. **Try it.** Three sliders labelled You, A, B, a Match/Miss toggle, and live score deltas beside each slider computed with `GameCore.settle`. Ties show all top bidders as buyers; an all-way tie shows "no trade". Caption: "Move the bids. Notice who wins the auction and who wins the money."
