...
- **Scope change 2026-09-13 (owner):** there is no background music. Task 11 ships sound effects only; the `audio` interface is `{ unlock, play, sfxEnabled, setSfx }`; the top bar has one sound toggle (`#sfxBtn`) and no `#musicBtn`; `app.js` makes no `musicPause` call. Any music text in the Task 9 or Task 11 code blocks below is superseded by this line, and Task 13's checklist item 5 reads "The sound toggle persists across reload."
...
7. Two tabs on the same seat: the taken-over view appears.
